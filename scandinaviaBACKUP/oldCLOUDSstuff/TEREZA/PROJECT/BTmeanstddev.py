...
(standard, mean) = meanstd(ch31bright_grid)

maxstd = np.nanmax(standard)
minstd = np.nanmin(standard)
maxmean = np.nanmax(mean)
minmean = np.nanmin(mean)
...
proj=make_plot(lcc_values)
x,y=proj(longrid,latgrid)
CS=proj.ax.pcolormesh(x,y,mean,cmap=cmap,norm=the_norm)
CBar=proj.colorbar(CS, 'right', size='5%', pad='5%',extend='both')
CBar.set_label('Mean 11 micron brightness temp (K) (3x3 box)')
proj.ax.set_title('Mean 11 micron brightness temp (K) (3x3 box)')
proj.ax.figure.canvas.draw()
fig.savefig('BoxMean.png')
...
proj=make_plot(lcc_values)
x,y=proj(longrid,latgrid)
CS=proj.ax.pcolormesh(x,y,standard,cmap=cmap,norm=the_norm)
CBar=proj.colorbar(CS, 'right', size='5%', pad='5%',extend='both')
CBar.set_label('Std deviation of 11 micron brightness temp (K) (3x3 box)')
proj.ax.set_title('Std deviation of 11 micron brightness temp (K) (3x3 box)')
proj.ax.figure.canvas.draw()
fig.savefig('BoxStandard.png')
